...
if (mysql_num_rows($result) > 0)
{
	...
	while($row = mysql_fetch_array($result))
	{
		$output .='<tr>
			...
			<td>'.$row["COST"].'</td>
			<td>'.$row["SKU"].'</td>
			<td><a href="../pages/product_master_edit.php?prodId='.$row["PRODUCT_ID"].'">Edit</a></td>
			<td><a href="../pages/update/product_master_delete.php?prodId='.$row["PRODUCT_ID"].'">Delete</a></td>
		</tr>
		';
	}
	echo $output;
